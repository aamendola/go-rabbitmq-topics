...
func (c Client) pp(consumer Consumer, deliveries <-chan amqp.Delivery, channel *amqp.Channel, keys []string) {
	for delivery := range deliveries {
		...
		if err != nil {
			next(delivery, channel, c.exchange, c.routingKeyErros, delivery.Timestamp)
		} else if requeue {
			delivery.Nack(false, true)
			log.Printf("[*] Requeue message")
		} else {
			if c.routingKeyTo != "" {
				next(delivery, channel, c.exchange, c.routingKeyTo, time.Now())
			} else {
...
func next(delivery amqp.Delivery, channel *amqp.Channel, exchange, routingKey string, timestamp time.Timestamp) {
	delivery.Ack(false)
