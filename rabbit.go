...
func (c Client) pp(deliveries <-chan amqp.Delivery, channel *amqp.Channel, keys []string) {
	for delivery := range deliveries {
		...
		err = c.Process(delivery.DeliveryTag, message)
		if err != nil {
			next(delivery, c.exchange, channel, c.routingKeyErros)
		} else {
			if c.routingKeyTo != "" {
				next(delivery, c.exchange, channel, c.routingKeyTo)
			} else {
