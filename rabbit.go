	...
	if !errorProcessor {
		...
		go func() {
			for delivery := range deliveries {
				...
				if c.blacklist != nil {
					processBlacklist(message, c.blacklist, delivery)
				} else {
...
func processBlacklist(message Message, blacklist []string, delivery amqp.Delivery) {
	exists := collections.Contains(blacklist, message.ID)
	if exists {
		delivery.Reject(false)
		Log(message.ID, fmt.Sprintf("[*] Message rejected"))
	} else {
		Log(message.ID, fmt.Sprintf("[*] Message does not belong to the blacklist", ))
	}
}
