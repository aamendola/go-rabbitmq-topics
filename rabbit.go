...
func (c Client) StartConsuming(consumer Consumer) {
	...
	errorProcessor := strings.Contains(c.queue, "_errors")
	...
	if !errorProcessor {
		...
		queue_errors_name := fmt.Sprintf("%s_errors", c.queue)
		routing_key_queue_errors := fmt.Sprintf("to_%s", queue_errors_name)
