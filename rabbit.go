...
// NewClient ...
func NewClient(logLevel, logFormat, host, user, password, exchange, queue, routingKeyFrom, routingKeyTo string, blacklist ...string) *Client {
...
// MakeClient ...
func MakeClient(logLevel, logFormat, host, user, password, exchange, queue, routingKeyFrom, routingKeyTo string, blacklist ...string) Client {
			...
			for delivery := range deliveries {
				...
				if c.blacklist != nil {
					...
					if exists {
						...
						Log(message.ID, fmt.Sprintf("[*] MessageId %s was rejected", message.ID))
						continue
					}
				}
		...
		go func() {
			...
			for delivery := range deliveries {
				...
				if c.blacklist != nil {
					...
					log.Printf("[+++] len(c.blacklist): %d", len(c.blacklist))
					exists := collections.Contains(c.blacklist, message.ID)
